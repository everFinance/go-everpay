...
import (
	...
	comm "github.com/everFinance/go-everpay/common"
	"github.com/everFinance/go-everpay/pay"
...
func (c *Cache) addTxsByAcc(tx *schema.TxResponse) {
	...
	if err != nil {
		log.Error("add to cache failed, IDCheck failed", "from", from, "err", err)
		return
	}
	...
	case paySchema.TxActionBundle:
		...
		if err = json.Unmarshal([]byte(tx.Data), &bundleData); err != nil {
			log.Error("can not unmarshal bundle data", "err", err, "everHash", tx.EverHash)
			return
		}
...
func GetMintTargetTxHash(everTxChainType, everTxData string) (targetChainTxHash string, err error) {
	targetChainType, err := pay.GetTargetChainTypeFromData(everTxData, tokSchema.TxActionMint, everTxChainType)
	if err != nil {
		return "", err
	}
