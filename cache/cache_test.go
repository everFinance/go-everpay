...
import (
	...
	"github.com/everFinance/go-everpay/cache/schema"
...
func TestInternalErrToStatus(t *testing.T) {
	...
	assert.Equal(t, "{\"status\":\"failed\",\"index\":1,\"msg\":\"aaa\"}", InternalErrToStatus(internalErr).Marshal())
}
...
func TestGetMintTargetTxHash(t *testing.T) {
	...
	assert.NoError(t, err)
	targetChainTxHash, err := GetMintTargetTxHash(mintArTx.ChainType, mintArTx.Data)
	assert.NoError(t, err)
	assert.Equal(t, "3aVtIp0afWPn_HDOp-EXGzhpFn2HEgTonDl753qVxTM", targetChainTxHash)
	...
	assert.NoError(t, err)
	targetChainTxHash, err = GetMintTargetTxHash(mintEthArTx.ChainType, mintEthArTx.Data)
	assert.NoError(t, err)
	assert.Equal(t, "0xfab7713ef2ff85ca205bac3a0bb536414454b5c72d5c90e21cef572e13663993", targetChainTxHash)
	...
	assert.NoError(t, err)
	targetChainTxHash, err = GetMintTargetTxHash(mintEthTx.ChainType, mintEthTx.Data)
	assert.NoError(t, err)
	assert.Equal(t, "0x860ca4d9d2332f0be07b25dde0628335636c42dc2e6d5f1d579d60ee2643c915", targetChainTxHash)
}
