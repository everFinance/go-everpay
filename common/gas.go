...
type Gas struct {
	gasFuncMap      map[string]func(url string) (fastestETH, fastETH, averageETH float64, err error) // key: url, val: get gasPrice func
...
func (g *Gas) Run() {
	...
	go func() {
		for {
			...
			if err := g.setGasPrice(); err != nil {
				sentry.CaptureException(fmt.Errorf("get gas price err: %v", err))
			}
		}
	}()
}
...
func GetGasBlockNative(url string) (fastestETH, fastETH, averageETH float64, err error) {
	...
	req.Header.Add("Authorization", "73a999cc-b2c7-45c1-83e8-ee71a4605b61") // TODO blocknative key
